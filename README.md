...
² A rotating textured cube built from the open [freegnm](https://gitgud.io/gluesniffer/freegnm-examples) toolchain (OpenOrbis has no PS4 shader compiler); exercises the full resource-bound shader path end-to-end. See [`_smoke/`](_smoke) for the graphics smoke configs.
...
- **Structured loops in the shader recompiler** — a GCN backward `s_branch` (which the linear, forward-only block emitter used to turn into an invalid SPIR-V back-edge) is now reconstructed into a proper `OpLoopMerge` loop: the header carries the loop merge, the back-edge routes through a synthetic continue block, and the loop-condition branch becomes a structured break. Natural single-level loops render correctly; more complex convergent control flow (a merge block shared by a loop and outer selections) still falls back to a loud "not implemented" rather than emitting invalid SPIR-V.
